...
export default function AcademicUnitHeader({
  eyebrow,
  title,
  description,
}: {
  ...
  return (
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <p className="text-sm font-medium text-[#4b91aa]">{eyebrow}</p>
      ...
      <div className="flex max-w-md items-start gap-3 rounded-2xl border border-[#cfe8f0] bg-[#f2fbfe] px-4 py-3 text-sm text-[#547783]">
        <Network className="mt-0.5 shrink-0 text-[#4e98b1]" size={19} />
        <p>ข้อมูลส่วนนี้ใช้กับรายการคณะและสาขาในหน้าสมัครสมาชิกนักเรียน</p>
      </div>
    </header>
  );
}
